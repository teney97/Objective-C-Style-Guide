...
简单来说 `default` 和 `@unknown default` 都可以用来处理已知以及未知的情况。区别在于，使用 `@unknown default`，如果你没有穷举所有枚举类型，或者未来有新增枚举类型，那么编译器会给出警告提示。关于选择应该是，对于非冻结枚举，如果你想穷举所有 case，并希望未来有新增枚举类型时得到编译器警告，那么就使用 `@unknown default`。
...
通过阅读本文，你是否对 Objective-C 的枚举宏有了更清晰的了解呢？`NS_CLOSED_ENUM` 用于声明不会变更枚举成员的冻结枚举，以降低灵活性的代价，换取了性能上的提升。`NS_STRING_ENUM/NS_EXTENSIBLE_STRING_ENUM`、`NS_TYPED_ENUM/NS_TYPED_EXTENSIBLE_ENUM` 用于声明字符串常量/类型常量枚举，这在混编时在 Swift 中使用起来更简洁优雅更 Swift。 
