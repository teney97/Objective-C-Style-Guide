...
通过阅读本文，你是否对 Objective-C 的枚举宏有了更清晰的了解呢？`NS_CLOSED_ENUM` 用于声明不会变更枚举成员的冻结枚举，以降低灵活性的代价，换取了性能上的提升。`NS_STRING_ENUM/NS_EXTENSIBLE_STRING_ENUM`、`NS_TYPED_ENUM/NS_TYPED_EXTENSIBLE_ENUM` 用于声明字符串常量/类型常量枚举，这在混编时在 Swift 中使用起来更优雅更 Swift。 
